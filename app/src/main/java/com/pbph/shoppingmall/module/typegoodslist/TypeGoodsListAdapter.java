...
public class TypeGoodsListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void setStrings(List<SearchProductResponse.DataBeanX.GoodsInfoBean.GoodsInfoListBean.DataBean> strings) {
        this.strings = strings;
        notifyDataSetChanged();
    }

    public void addStrings(List<SearchProductResponse.DataBeanX.GoodsInfoBean.GoodsInfoListBean.DataBean> strings) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (type == 0) {
            ...
            linearViewHolder.tvPrice.setText(String.valueOf(dataBean.getCommcont()));
            linearViewHolder.tvPrice.append("条评价,");
            linearViewHolder.tvPrice.append(String.valueOf(dataBean.getPraise()));
            linearViewHolder.tvPrice.append("%好评");
            linearViewHolder.relative_view.setOnClickListener(new OnSPClickListener() {
                @Override
            ...
            if (dataBean.getImgList().size() != 0) {
                Glide.with(context)
                        .load(dataBean.getImgList().get(0).getImageArtworkName())
                        .error(R.drawable.wuliu_140x140)
                        .into(linearViewHolder.iv);
            }
        } else {
            ...
            gridViewHolder.tv_content.setText(String.valueOf(dataBean.getCommcont()));
            gridViewHolder.tv_content.append("条评价,");
            gridViewHolder.tv_content.append(String.valueOf(dataBean.getPraise()));
            gridViewHolder.tv_content.append("%好评");

            gridViewHolder.relative_view.setOnClickListener(new OnSPClickListener() {
                @Override
                public void onClickSucc(View v) {
                    if (dataBean == null) return;
                    if (dataBean.getGoodsInfoId() == -1) return;
    ...
    public interface TransmitData {
        void transmitGoodsId(int goodsInfoId);
    }
