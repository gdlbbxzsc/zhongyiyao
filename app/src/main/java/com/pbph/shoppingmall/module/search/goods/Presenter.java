package com.pbph.shoppingmall.module.search.goods;


import com.pbph.mvp.base.mvp.BasePresenter;
import com.pbph.mvp.rxbus2.RxBusF;
import com.pbph.shoppingmall.model.message.SearchPriceMsg;
import com.pbph.shoppingmall.model.message.SearchShowDrawerMsg;
import com.pbph.shoppingmall.model.message.SearchText2GoodsItemMsg;
import com.pbph.shoppingmall.model.message.SearchTextMsg;

public class Presenter<T extends Contract.View> extends BasePresenter<T> implements Contract.Presenter {

    String searchText;

    public Presenter(T baseView) {
        super(baseView);
    }

    @Override
    public void subscribe() {  //  第一次创建默认执行的事情
        subscribeRxBus2getSearchText();
    }

    @Override
    public void unsubscribe() { //销毁时执行的事情
        unSubscribeRxBus2getSearchText();
    }


    @Override
    public void subscribeRxBus2getSearchText() {
        RxBusF.register0(Presenter.this, SearchTextMsg.class, it -> {
            searchText = it.searchText;

            postRxBus4SendSeachText2Item(true);
        });
    }

    @Override
    public void unSubscribeRxBus2getSearchText() {
        RxBusF.removeDisposable0(Presenter.this, SearchTextMsg.class);
    }

    SearchPriceMsg searchPriceMsg = new SearchPriceMsg();

    @Override
    public void postRxBus4flushListByPrice(boolean type) {
        searchPriceMsg.priceType = type;
        RxBusF.post0(searchPriceMsg);
    }

    SearchShowDrawerMsg searchShowDrawerMsg = new SearchShowDrawerMsg();

    @Override
    public void postRxBus4showDrawer() {
        RxBusF.post0(searchShowDrawerMsg);
    }


    SearchText2GoodsItemMsg searchTextMsg = new SearchText2GoodsItemMsg();

    @Override
    public void postRxBus4SendSeachText2Item(boolean isSearchTextChanged) {
        searchTextMsg.searchText = searchText;
        searchTextMsg.isSearchTextChanged = isSearchTextChanged;
        RxBusF.post0(searchTextMsg);
    }

}
